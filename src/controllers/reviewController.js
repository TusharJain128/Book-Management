...
const createReview = async function (req, res) {
    try {
        ...
        if (reviewedAt) {
            if (!valid.isValidDate(reviewedAt)) return res.status(400).send({ status: false, message: "ReviewBy should be String" })
        }

        let bookData= await booksModel.findByIdAndUpdate(bookId, { $inc: { reviews: 1 } }) 

        let savedData = await reviewModel.create(data)
        
        let response = bookData._doc
        
        response.reviewsData = savedData
        ...
        res.status(201).send({ status: true, message: "Books list", data: response })
...
const reviewUpdate = async function (req, res) {
    try {
        ...
        if(Object.keys(req.body).length===0) return res.status(400).send({status:false,message:"Please enter some data to update"})
